...
class ContainerManager {
    sessionContainerPrefix = 'lifeos-webrtc-browser-';
    dockerHost = process.env.STREAM_MANAGER_INTERNAL_HOST || '127.0.0.1';
    dockerNetwork = process.env.STREAM_MANAGER_DOCKER_NETWORK || '';
    runtimePath = process.env.STREAM_MANAGER_RUNTIME_PATH || '/opt/lifeos-runtime';
    runtimeVolumeName = process.env.STREAM_MANAGER_RUNTIME_VOLUME_NAME || 'lifeos-stream-runtime';
    childRuntimePath = process.env.STREAM_MANAGER_CHILD_RUNTIME_PATH || '/opt/lifeos-runtime';
    publicBaseUrl = process.env.STREAM_PUBLIC_BASE_URL ||
        process.env.FRONTEND_URL ||
        'http://127.0.0.1';
    viewerHost = process.env.STREAM_MANAGER_DIRECT_VIEWER_HOST || '127.0.0.1';
    publicIp = process.env.STREAM_WEBRTC_PUBLIC_IP || this.derivePublicIpFromBaseUrl();
    sessions = new Map();
    imageName = process.env.STREAM_BROWSER_IMAGE || 'ghcr.io/m1k1o/neko/chromium:latest';
    nextViewerPort = 18080;
    nextTcpMuxPort = 53000;
    nextUdpMuxPort = 54000;
    maxPortOffset = 99;
    ...
    // --------------------------------------------
    async startContainer(sessionId, appName, url, resolution = '1600x900x24') {
        if (!this.dockerAvailable) {
            return {
                sessionId,
                containerId: '',
        ...
        const internalViewerBaseUrl = this.buildInternalViewerBaseUrl(containerName, viewerPort);
        const session = {
            sessionId,
            containerId: '',
            containerName,
        ...
        try {
            ...
            console.log(`[ContainerManager] Starte WebRTC-Browser fuer "${appName}" (${targetUrl})...`);
            console.log(`[ContainerManager] Ports: Viewer=${viewerPort}, TCPMUX=${tcpMuxPort}, UDPMUX=${udpMuxPort}`);
            ...
            const publicIpEnv = this.publicIp
                ...
                : '';
            const command = `docker run -d --rm --platform linux/amd64 ` +
                `--name ${containerName} ` +
                ...
                `${publicIpEnv}` +
                `${this.imageName} ` +
                `sh -lc ${this.escapeShellArg(`cp "${runtimeSessionPath}/chromium.conf" /etc/neko/supervisord/chromium.conf && ` +
            ...
            console.log(`[ContainerManager] "${appName}" laeuft! Viewer: ${session.viewerUrl}`);
            return session;
        }
        catch (error) {
    ...
    getViewerProxyTarget(sessionId) {
        const session = this.sessions.get(sessionId);
        if (!session) {
            return null;
        }
    ...
    async stopAll() {
        const promises = Array.from(this.sessions.keys()).map((id) => this.stopContainer(id));
    ...
    // --------------------------------------------
    async prepareRuntimeFiles(session) {
        const sessionRuntimeDir = join(this.runtimePath, session.sessionId);
        await mkdir(sessionRuntimeDir, { recursive: true });
        await writeFile(join(sessionRuntimeDir, 'chromium.conf'), this.buildChromiumSupervisorConfig(session), 'utf8');
    }
    async cleanupRuntimeFiles(sessionId) {
    ...
    // --------------------------------------------
    async waitForReady(session, timeoutMs) {
        ...
        while (Date.now() - startTime < timeoutMs) {
            try {
                ...
                if (response.ok) {
                    console.log(`[ContainerManager] WebRTC-Viewer bereit auf ${viewerUrl}`);
                    return;
                }
            }
            catch {
        ...
        throw new Error(`WebRTC-Browser nicht bereit (Timeout, URL: ${viewerUrl}/)`);
    }
    // --------------------------------------------
    ...
    cleanupOldContainers() {
        try {
            execSync(`docker ps -aq --filter "name=${this.sessionContainerPrefix}" | xargs docker rm -f 2>/dev/null || true`, { timeout: 10000, stdio: 'ignore' });
    ...
    // --------------------------------------------
    buildChromiumSupervisorConfig(session) {
...
`;
    ...
    // --------------------------------------------
    buildDirectViewerUrl(viewerPort, userName, userPassword) {
        try {
            ...
            return `http://${this.viewerHost}:${viewerPort}/${this.buildViewerQuery(userName, userPassword)}`;
        }
    }
    buildViewerQuery(userName, userPassword) {
    ...
    buildInternalViewerBaseUrl(containerName, mappedPort) {
