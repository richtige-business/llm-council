...
// --------------------------------------------
export async function getWindowInfo(appName) {
    try {
...
`;
        const { stdout } = await execAsync(`echo ${JSON.stringify(swiftCode)} | swift -`, { timeout: 12000 });
...
// --------------------------------------------
export async function getAllWindows(appName) {
    try {
        const swiftCode = `
import CoreGraphics
import Foundation
...
// --------------------------------------------
export async function hideWindow(appName) {
    try {
...
// --------------------------------------------
export async function showWindow(appName) {
    try {
        const script = `
tell application "System Events"
  if exists process "${appName}" then
...
// --------------------------------------------
export async function waitForWindow(appName, maxWaitMs = 15000) {
...
// --------------------------------------------
export async function hideAppWindow(appName) {
    try {
