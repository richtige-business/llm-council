...
function sendError(res, status, code, message) {
    const error = { error: code, message, code };
...
app.post('/api/app/focus', async (req, res) => {
    try {
        const { appName, sessionId } = req.body;
        if (!appName || !sessionId) {
            return sendError(res, 400, 'MISSING_PARAMS', 'appName und sessionId sind erforderlich');
...
async function extractAppIcon(appName) {
    ...
    try {
        ...
        try {
            const plistOutput = execSync(`defaults read "${appPath}/Contents/Info" CFBundleIconFile 2>/dev/null || echo ""`, { encoding: 'utf-8' }).trim();
            if (plistOutput) {
        ...
        if (!icnsPath) {
            ...
            try {
                const found = execSync(`find "${appPath}/Contents/Resources" -name "*.icns" -maxdepth 1 2>/dev/null | head -1`, { encoding: 'utf-8' }).trim();
                if (found)
                    icnsPath = found;
            }
            catch {
        ...
        execSync(`sips -s format png "${icnsPath}" --out "${cachedPath}" --resampleWidth 128 2>/dev/null`, { encoding: 'utf-8' });
        if (existsSync(cachedPath)) {
...
// --------------------------------------------
app.get('/api/system/installed-apps', async (_req, res) => {
    try {
        ...
        const sortedApps = Array.from(apps).sort((a, b) => a.toLowerCase().localeCompare(b.toLowerCase()));
...
// --------------------------------------------
app.get('/api/system/app-icon', async (req, res) => {
    try {
...
app.get('/api/window/info', async (req, res) => {
...
app.get('/api/window/list', async (req, res) => {
    const appName = req.query.appName;
    if (!appName) {
        return res.status(400).json({ error: 'MISSING_APP_NAME', message: 'appName Query-Parameter fehlt' });
    }
...
app.post('/api/window/hide', async (req, res) => {
    const { appName, sessionId } = req.body;
    if (!appName) {
...
app.post('/api/window/show', async (req, res) => {
...
app.post('/api/capture/start', async (req, res) => {
    ...
    try {
        const session = await captureManager.startCapture(sessionId, Number(windowId), appName || 'Unknown', fps ? Number(fps) : 30, bitrate ? Number(bitrate) : 5_000_000);
...
// --------------------------------------------
app.post('/api/app/launch-and-stream', async (req, res) => {
    ...
    try {
        ...
        const captureSession = await captureManager.startCapture(sessionId, windowInfo.windowId, appName, fps ? Number(fps) : 30, bitrate ? Number(bitrate) : 5_000_000);
...
app.post('/api/container/start', async (req, res) => {
    ...
    try {
        ...
        const session = await containerManager.startContainer(sessionId, appName, url, resolution || '1920x1080x24');
...
// --------------------------------------------
app.get('/api/container/status/:sessionId', (req, res) => {
...
// --------------------------------------------
app.get('/api/container/apps', (_req, res) => {
    return res.json({
        success: true,
...
// --------------------------------------------
app.get('/api/container/sessions', (_req, res) => {
    return res.json({
        success: true,
