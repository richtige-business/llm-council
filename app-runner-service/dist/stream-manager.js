...
app.post('/api/container/start', async (req, res) => {
    ...
    try {
        const session = await containerManager.startContainer(sessionId, appName, url, resolution || '1600x900x24');
...
// --------------------------------------------
app.get(['/stream/:sessionId', '/stream/:sessionId/'], async (req, res) => {
...
app.use('/stream/:sessionId', (req, res) => {
    const proxyTarget = buildViewerProxyUrl(req.originalUrl, req.params.sessionId);
    if (!proxyTarget) {
        return res.status(404).json({
            error: 'STREAM_NOT_FOUND',
...
// --------------------------------------------
function buildViewerProxyUrl(requestUrl, routeSessionId) {
    try {
        ...
        const forwardedPath = pathParts.length > 2 ? `/${pathParts.slice(2).join('/')}` : '/';
