...
// Beschreibt den Zustand einer aktiven Capture-Session
// --------------------------------------------

export interface CaptureSession {
  sessionId: string;            // Session-ID
  windowId: number;             // CGWindowID des Zielfensters
  appName: string;              // Name der gecaptureten App
  helperPort: number;           // WebSocket-Port des Swift Helpers
  helperPid: number | null;     // PID des Helper-Prozesses
  status: 'starting' | 'running' | 'stopped' | 'error';
  startedAt: number;            // Unix Timestamp
  lastError: string | null;     // Letzter Fehler
  fps: number;                  // Konfigurierte Framerate
  bitrate: number;              // Konfigurierte Bitrate
}
...
class CaptureManager {
  ...
  private sessions: Map<string, { session: CaptureSession; process: ChildProcess | null }> = new Map();

  // Pfad zum Swift Helper Binary
  private helperPath: string;

  // Port-Range fuer Helper WebSocket Server (9090-9099)
  private nextPort = 9090;
  private readonly maxPort = 9099;
  ...
  private cleanupOldHelpers(): void {
    try {
  ...
  private handleHelperMessage(sessionId: string, msg: { type: string; [key: string]: unknown }): void {
    const entry = this.sessions.get(sessionId);
    if (!entry) return;
    ...
    switch (msg.type) {
      ...
      case 'error':
        ...
        entry.session.status = 'error';
        entry.session.lastError = msg.message as string;
        break;
  ...
  private async waitForHelperReady(sessionId: string, timeoutMs: number): Promise<void> {
    ...
    while (Date.now() - startTime < timeoutMs) {
      ...
      if (entry.session.status === 'error') return;
      if (entry.session.status === 'stopped') return;
