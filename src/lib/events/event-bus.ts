...
import type { LifeOSEvent, EventHandler } from '@/types';
...
interface EventBusActions {
  ...
  emit: <T>(event: Omit<LifeOSEvent<T>, 'timestamp'>) => void;
  ...
  getHistory: (limit?: number) => LifeOSEvent[];
...
export const useEventBus = create<EventBusState & EventBusActions>((set, get) => ({
  ...
  // --------------------------------------------
  emit: <T>(event: Omit<LifeOSEvent<T>, 'timestamp'>) => {
    // Vollständiges Event mit Timestamp erstellen
    const fullEvent: LifeOSEvent<T> = {
      ...event,
      timestamp: Date.now(),
    };
    ...
    set((state) => ({
      history: [...state.history.slice(-99), fullEvent as LifeOSEvent],
    }));
    ...
    exactHandlers?.forEach(handler => {
      try {
        handler(fullEvent as LifeOSEvent);
      } catch (error) {
